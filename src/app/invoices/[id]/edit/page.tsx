...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const invoice = (await invoicesAPI.getById(params.id as string)).data as Invoice;
        setFormData({
        ...
        setNotification({ type: 'error', message: 'Failed to load invoice' });
      } finally {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await invoicesAPI.update(params.id as string, invoiceData);
            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              ...
              <div>
                ...
                {existingAttachment && !removeAttachment ? (
                  <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
                    <p>File lama: {existingAttachment.name}</p>
                ...
                {removeAttachment ? (
                  <p className="mt-1 text-xs font-medium text-amber-600">
                    File lama akan dihapus saat update disimpan.
                  </p>
                ) : null}
                {attachmentFile ? (
                  <p className="mt-1 text-xs font-medium text-slate-700 dark:text-slate-200">
                    File baru: {attachmentFile.name}
                  </p>
                ) : null}
                {attachmentError ? (
              ...
              <div className="flex gap-3 pt-4">
                <button
                  type="submit"
                  disabled={loading}
                  className="btn-update flex items-center gap-2"
                >
                  <Save size={16} />
